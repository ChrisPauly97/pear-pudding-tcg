    ...
    if (draggingCard != null) {
      ...
      if (activePlayer.hasEnoughMana(draggingCard) && draggingCard.getCurrentLocation() == HAND) {
        ...
        switch (draggingCard.getStatusEffect().getEffectTrigger()) {
          ...
          case SUMMON:
            // Check which effect type we have?
            // In all summon cases we want to spend the mana, and add the card to our board
            switch (draggingCard.getStatusEffect().getEffectType()) {
              case REMOVE:
                if (enemyBoardTargetSlot != -1) {
                  var enemyTargetCard = enemyBoard.getCardAtIndex(enemyBoardTargetSlot);
                  if (enemyTargetCard != null) {
                    Gdx.app.log("Enemy", draggingCard.getCurrentLocation() + " " + enemyBoardTargetSlot);
                    enemyTargetCard.handleRemoveEffect(draggingCard.getStatusEffect().getValue());
                ...
                if (boardTargetSlot != -1) {
                  var friendlyTargetCard = playerBoard.getCardAtIndex(boardTargetSlot);
                  if (friendlyTargetCard != null) {
                    Gdx.app.log("Board", draggingCard.getCurrentLocation() + " " + boardTargetSlot);
                    friendlyTargetCard.handleRemoveEffect(draggingCard.getStatusEffect().getValue());
                    playerBoard.addCard(draggingCard);
                    activePlayer.setCurrentMana(activePlayer.getCurrentMana() - draggingCard.getCost());
                  }
                }
              ...
              case DAMAGE:
                if (enemyBoardTargetSlot != -1) {
                  ...
                  if (enemyTargetCard != null) {
                    Gdx.app.log("Enemy", draggingCard.getCurrentLocation() + " " + enemyBoardTargetSlot);
                    enemyTargetCard.takeDamage(draggingCard.getStatusEffect().getValue());
                    playerBoard.addCard(draggingCard);
                    activePlayer.setCurrentMana(activePlayer.getCurrentMana() - draggingCard.getCost());
                  }
                }
              ...
              case HEAL:
                if (enemyBoardTargetSlot != -1) {
                  ...
                  if (enemyTargetCard != null) {
                    Gdx.app.log("Enemy", draggingCard.getCurrentLocation() + " " + enemyBoardTargetSlot);
                    enemyTargetCard.getHealing(draggingCard.getStatusEffect().getValue());
                    playerBoard.addCard(draggingCard);
                    activePlayer.setCurrentMana(activePlayer.getCurrentMana() - draggingCard.getCost());
                  }
                }
